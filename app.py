...
# Ukrywanie domyślnego menu, przycisku Deploy i stopki Streamlit
hide_st_style = """
            <style>
...
# ==========================================
LOT_PRICE_USD = 500_000.0  # Stała cena 1 Lota dla XAUUSD
...
def load_default_order_book_spot() -> pd.DataFrame:
    """Nowy, domyślny Order Book dla zakładki Spot (z pliku OB.xlsx)"""
    return pd.DataFrame({
...
def render_dashboard(vol_dist_df: pd.DataFrame, tab_name: str, default_ob_df: pd.DataFrame) -> None:
    ...
    with col_left:
        ...
        if errors_a:
            for err in errors_a: st.error(f"Order Book A — {err}")
            return
    ...
    with col_right:
        ...
        if errors_b:
            for err in errors_b: st.error(f"Order Book B — {err}")
            return

        results_b = calculate_per_bucket_revenue(edited_ob_b, vol_dist_df)

        if results_b.empty:
            st.warning("Brak wyników dla Scenariusza B.")
            return
    ...
    fig_fill.add_trace(go.Bar(
        x=fill_a["OB Line"].astype(str), y=fill_a["Fill Volume (%)"],
        name="Fill Volume % — A", marker_color="#5B9BD5", opacity=0.85,
    ), secondary_y=False)

    fig_fill.add_trace(go.Bar(
        x=fill_b["OB Line"].astype(str), y=fill_b["Fill Volume (%)"],
        name="Fill Volume % — B", marker_color="#70AD47", opacity=0.85,
    ), secondary_y=False)

    fig_fill.add_trace(go.Scatter(
        x=fill_a["OB Line"].astype(str), y=fill_a["Fill Count"],
        name="Fill Count — A", mode="lines+markers", marker_color="#EF553B", line=dict(width=2, dash="dot"),
    ), secondary_y=True)

    fig_fill.add_trace(go.Scatter(
        x=fill_b["OB Line"].astype(str), y=fill_b["Fill Count"],
        name="Fill Count — B", mode="lines+markers", marker_color="#FFA15A", line=dict(width=2, dash="dash"),
    ), secondary_y=True)
    ...
    fig_ob.add_trace(go.Bar(
        x=ob_lines_str, y=ask_a[:n], name="Current (Ask Size)", marker_color="#5B9BD5", opacity=0.85,
    ), row=1, col=1)

    fig_ob.add_trace(go.Bar(
        x=ob_lines_str, y=ask_b[:n], name="Optimized (Ask Size)", marker_color="#70AD47", opacity=0.85,
    ), row=1, col=1)

    # USUNIĘTE CZERWONE/RÓŻOWE TŁO DLA LINII 1-2

    fig_ob.add_trace(go.Scatter(
        x=ob_lines_str, y=spr_a[:n], name="Current (Spread)", mode="lines+markers",
        marker=dict(symbol="circle", size=8, color="#5B9BD5"), line=dict(color="#5B9BD5", width=2),
    ), row=1, col=2)

    fig_ob.add_trace(go.Scatter(
        x=ob_lines_str, y=spr_b[:n], name="Optimized (Spread)", mode="lines+markers",
        marker=dict(symbol="square", size=8, color="#375623"), line=dict(color="#375623", width=2),
    ), row=1, col=2)

    fig_ob.update_layout(
        barmode="group", hovermode="x unified", height=420,
        margin=dict(l=0, r=0, t=60, b=0), legend=dict(orientation="h", yanchor="bottom", y=1.08, xanchor="right", x=1),
    )
    ...
    fig_rev.add_trace(go.Scatter(
        x=results_b["Volume_Bucket"], y=results_b["Pct_Diff"],
        name="Różnica B vs A (%)", mode="lines+markers", marker_color="#FFA15A",
        line=dict(width=3, dash="dot"),
    ), secondary_y=True)

    fig_rev.update_layout(
        barmode="group", xaxis_title="Przedział Wolumenu (Volume Bucket)", hovermode="x unified",
        margin=dict(l=0, r=0, t=40, b=0), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
    )
    fig_rev.update_yaxes(title_text="Przychód (USD)", secondary_y=False)
    fig_rev.update_yaxes(title_text="Zmiana (%)", secondary_y=True, showgrid=False, tickformat=".1f", ticksuffix="%")
